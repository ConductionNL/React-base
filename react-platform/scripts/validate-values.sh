...
for f in "${files[@]}"; do
  ...
  # Convention check: filename should match tenant.name.
  expected="tenant-${name}.yaml"
  if [[ -n "$name" && "$base" != "$expected" ]]; then
    echo "[FAIL] $base: filename should be '$expected' to match .tenant.name"
    fail=1
  fi
